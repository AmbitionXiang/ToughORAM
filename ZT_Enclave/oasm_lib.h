...
/*
	...
	*/
extern "C" void oblock_move_on_flag(Block *res_blk, Block *iter_blk, uint32_t data_size, bool flag);
...
/*
		...
		On flag :
			- copy gN to block_id
	*/
extern "C" void oset_block_as_dummy(uint32_t *block_id, uint32_t gN, uint32_t flag);

extern "C" void ostore_deepest(uint32_t treeLabel, uint32_t leaf, uint32_t *level_deepest, uint32_t blocksize);
...
/*
	...
	*/
extern "C" void pd_setdeepest(uint32_t *deepest, int32_t src, uint32_t flag);

extern "C" void pt_settarget(uint32_t *target, int32_t *dest, int32_t *src, uint32_t flag);

extern "C" void pt_set_target_position(int32_t *target_position, uint32_t pos_in_bucket, uint32_t *target_position_set_flag, uint32_t flag);
...
/*
		...
		On flag:
			...
			- copy i -> dest
	*/
extern "C" void pt_set_src_dest(int32_t *src, int32_t *dest, int32_t deepest_of_i_1, uint32_t i, uint32_t flag_pt);
/*
		oset_return_value
		On Flag:
			-
...
extern "C" void oblock_move_to_bucket(unsigned char *serialized_block, Block *bucket_block, uint32_t extdata_size, bool flag, bool *written, uint32_t *posk);

extern "C" void oset_goal_source(uint32_t level_in_path, uint32_t local_deepest_in_path, uint32_t flag, int32_t *src, int32_t *goal);
extern "C" void oset_hold_dest(int32_t *hold, int32_t *dest, uint32_t *wflag, uint32_t flag);
